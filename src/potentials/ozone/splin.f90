...
! step 1: read xi and yi from tabCn-roo.res, xmin, xmax, n
  open (unit=1, file='tabCn-roo.res',status='old')
  read(1,*)
  do nl=1,nlt
  read(1,*)(ll(nl,i),i=1,6),(coef(nl,n),n=1,nr)
  enddo
   close(1)
    open(unit=2,file='PolarO2_D2h_aVQZ_roo.res',status='old')
    read(2,*)
    read(2,*)
    do n=1,nr
    read(2,*)rro(n),yy,zz,qq2(n)
    enddo
    close(2)
